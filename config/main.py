...
def get_deepspeed_config(precision='fp16'):
    ...
    if global_args["enable_lora"] or global_args["enable_ptv2"]:
        ...
        filename = os.path.join(os.path.dirname(__file__), 'deepspeed_offload.yaml')
    else:
        filename = os.path.join(os.path.dirname(__file__), 'deepspeed.yaml')

    deepspeed_config = load_yaml(filename)
